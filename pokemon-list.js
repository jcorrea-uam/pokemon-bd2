    ...
    const list = pokemon.map((p) => `
        <div class="card" style="width: 12rem;">
            ...
            <ul class="list-group list-group-flush">
                ...
                <li class="list-group-item">${p.types.join(', ')}</li>
            </ul>
            <div class="card-body">
...
function listEvolutions(pokemon) {
    const body = pokemon.map((p) => `
        <tr>
            <td><img data-pid="${p.id}" src="${p.thumbnail}" draggable="true" ondragstart="drag(event)" class="card-img-top rounded mx-auto d-block" style="width: 50px;" alt="${p.name}'s picture"></td>
            <td>${p.name}</td>
        </tr>
    `).join("\n");
